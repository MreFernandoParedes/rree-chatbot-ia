...
import ChatManager from "./ChatHistory/ChatManager";
...
const Chat = () => {
    ...
    const handleAsyncRequest = async (question: string, answers: [string, ChatAppResponse][], responseBody: ReadableStream<any>) => {
        ...
        try {
            ...
            for await (const event of readNDJSONStream(responseBody)) {
                if (event["context"] && event["context"]["data_points"]) {
                    ...
                    if (event.context.completion_tokens) completionTokens = event.context.completion_tokens;
                    if (event.context.prompt_tokens) promptTokens = event.context.prompt_tokens;
                    if (event.context.total_tokens) totalTokens = event.context.total_tokens;
                } else if (event["error"]) {
        ...
        // console.log("chatQuestion:", question);
        // console.log("chatAnswer:", fullResponse.message.content);
        ChatManager({
            chatQuestion: question,
            chatAnswer: fullResponse.message.content,
    ...
    //         const available = await checkTokenLimit();
        ...
        <div className={styles.container}>
            ...
            <div>
                <div className={styles.commandsContainer}>
                    <img className={`${styles.centeredLogo}`} src={Logo} alt="Logo de Ministerio de Relaciones Exteriores"></img>
                    {showClearButton && (
                        <ClearChatButton className={styles.commandButton} onClick={clearChat} disabled={!lastQuestionRef.current || isLoading} />
                    )}
                    {showUserUpload && <UploadFile className={styles.commandButton} disabled={!loggedIn} />}
                    {/* CONFIGURACIÓN: Ocultar botón "Configuración del desarrollador" */}
                ...
                <div className={styles.chatContainer}>
                    {!lastQuestionRef.current ? (
                        <div className={styles.chatEmptyState}>
                            ...
                            <h4 className={styles.chatEmptyStateSubtitle}>{t("chatEmptyStateSubtitle")}</h4>
                    ...
                    <div className={styles.chatInput}>
                        <QuestionInput
                            clearOnSend
                            // placeholder={isAvailable ? t("defaultExamples.placeholder") : "Límite de interacciones alcanzado"}
                        ...
                        {/* <QuestionInput
                            clearOnSend
                            placeholder={isAvailable ? t("defaultExamples.placeholder") : "Resource limit reached"}
                            disabled={isLoadingLimit || !isAvailable}
                            onSend={question => {
                                if (isAvailable) {
                    ...
                    {/* <div className={styles.chatBottomLine}></div> */}
                </div>
