...
// Agregar un Promise Promise<ResponseLimit>
export async function checkInteractionsLimit() {
    const url = `${BACKEND_URI}/interactions_limit`;
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error("Failed to fetch interactions limit status");
        }

        const data = await response.json();
        // console.log("available = ", data.available);
        return data.available; // Retorna solo el valor de "available" (True o False)
    } catch (error) {
        console.error("Error checking interactions limit:", error);
        return null;
    }
}
