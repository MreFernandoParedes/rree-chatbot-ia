...
from chats.chat_service import register_chat, is_limit_reached
...
@bp.route("/storechat", methods=["POST"])
async def save_chat():
    chat_data = await request.get_json()
    return await register_chat(chat_data)


@bp.route("/interactions_limit", methods=["GET"])
async def interactions_limit():
    return await is_limit_reached()
